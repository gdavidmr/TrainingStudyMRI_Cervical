...
jobfile = {'G:\projects\03_Scripts\dmri_cervical\training_batch_sc_dwi_dti_rob.mat'};

for i = 1:length(data)

    display(data{i});
    
    % Get processed dMRI volumes
    dwi = dir([data{i} filesep 'dmri_cervical' filesep 'derivatives' filesep 'msPOAS' filesep 'sub-*_ses-*_acq-dmri-cervical_desc-CROP-ECMOCO-msPOAS_dwi.nii']);
    dwi = fullfile(dwi(1).folder, dwi(1).name);
    dwi = [dwi ',1'];

    % Get bvals and bvecs
    bvals = dir('W:\Forschung-ZfP\04_MRI\03_Training\03_Scripts\dmri_cervical\diffpars_cervical_training.bval');
    bvals = fullfile(bvals(1).folder, bvals(1).name);
    
    bvecs = dir('W:\Forschung-ZfP\04_MRI\03_Training\03_Scripts\dmri_cervical\diffpars_cervical_training.bvec');
    bvecs = fullfile(bvecs(1).folder, bvecs(1).name);
    
    bvals_bvecs(1,:) = bvals;
    bvals_bvecs(2,:) = bvecs;
    
    % Get noise standard deviation (sigma estimate)
    sigma = dir([data{i} filesep 'dmri_cervical' filesep 'derivatives' filesep 'SIGMA' filesep 'sub-*_ses-*_acq-dmri-cervical_desc-CROP-ECMOCO-SIGMA.txt']);
    sigma = fullfile(sigma(1).folder, sigma(1).name);
    
    % Get spinal cord mask
    mask_sc = dir([data{i} filesep 'dmri_cervical' filesep 'derivatives' filesep 'label' filesep 'sub-*_ses-*_acq-dmri-cervical_desc-CROP-ECMOCO-mean-b500-sc_mask.nii']);
    mask_sc = fullfile(mask_sc.folder, mask_sc.name);

    % Run batch file
    try
        inputs = cell(4,1);
        inputs{1,1} = cellstr(dwi);
        inputs{2,1} = cellstr(bvals_bvecs);
        inputs{3,1} = cellstr(sigma);
        inputs{4,1} = cellstr(mask_sc);
